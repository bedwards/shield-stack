...
## 2026-03-19 | multiple | Merger: verify actual code before blocking — bot reviews and prior merger reviews can have false positives (PRs #343, #352)
**What happened:** PR #343 was blocked for "hydration mismatch" but the code already had the `mounted` guard fix. PR #352 was blocked for "seed data commented out" and "missing GRANT" but the seed data was active (lines 40-123) and GRANT statements were present (lines 293-294). Both PRs were blocked for 2+ review cycles based on incorrect assessments, including a previous merger review that accepted the Gemini findings without verifying the actual code.
**Rule:** Before posting a BLOCKED review, the merger MUST read the actual source files in the worktree/branch — not just trust bot review summaries or prior merger reviews. Run `git show origin/{branch}:{file}` or read from the worktree to verify the specific lines cited. Bot reviews can reference stale line numbers or misread code. Prior merger reviews can propagate false positives. The merger is the final quality gate and must independently verify.

## 2026-03-19 | cellscore | WIP PRs with `wip()` prefix: merge to preserve salvaged work, don't block indefinitely (PR #362)
**What happened:** PR #362 was blocked twice for 3 HIGH findings (missing API route, direct affiliate URLs, no Mapbox geocoder). These were real gaps but represented unfinished features in explicitly labeled WIP/salvaged work from an expired context window. The original worker context was gone. A third blocking review would have left the PR stuck forever with 1839 lines of solid work unmerged.
**Rule:** WIP PRs (title prefix `wip()`) represent salvaged partial work from expired context windows. When HIGH findings are "features not yet implemented" rather than "bugs in existing code," and the product is not deployed, merge the WIP and keep the parent issue open for follow-up. Blocking WIP PRs repeatedly when no worker can fix them wastes review cycles. Distinguish: (a) bugs in shipped code → BLOCK, (b) missing features in undeployed WIP → merge with documented gaps. Consistent with PRs #359, #360 which followed the same pattern.
