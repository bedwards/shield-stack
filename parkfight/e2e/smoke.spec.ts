...
test.describe("ParkFight Smoke Tests", () => {
  ...
  test("navigation links are present", async ({ page }) => {
    ...
    await expect(page.getByTestId("nav-fight")).toBeVisible();
    await expect(page.getByTestId("nav-cities")).toBeVisible();
  ...
  test("page title is correct", async ({ page }) => { await page.goto("/"); await expect(page).toHaveTitle(/ParkFight/); });
});
