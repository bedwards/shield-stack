...
test.describe("SettleScan Smoke Tests", () => {
  test("landing page loads and displays hero", async ({ page }) => {
    ...
    await expect(page.getByTestId("cta-match-button")).toBeVisible();
  ...
  test("navigation links are present", async ({ page }) => {
    ...
    await expect(page.getByTestId("nav-settlements")).toBeVisible();
    await expect(page.getByTestId("nav-match")).toBeVisible();
  ...
  test("page title is correct", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveTitle(/SettleScan/);
  });
});
